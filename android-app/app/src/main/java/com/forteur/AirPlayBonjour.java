...
public class AirPlayBonjour {
    ...
    public void start(int airPlayPort, int airTunesPort) throws Exception {
        // Usa un IP esplicito se necessario
        InetAddress address = InetAddress.getLocalHost(); // Ottieni l'indirizzo locale

        // Registro servizio AirPlay
        airPlayService = ServiceInfo.create(AIRPLAY_SERVICE_TYPE, serverName, airPlayPort, 0, 0, airPlayMDNSProps());
        JmmDNS.Factory.getInstance().registerService(airPlayService);  // Registra il servizio su un indirizzo specifico
        log.info("{} service is registered on port {}", serverName + AIRPLAY_SERVICE_TYPE, airPlayPort);

        // Registro servizio AirTunes
        String airTunesServerName = "010203040506@" + serverName;
        airTunesService = ServiceInfo.create(AIRTUNES_SERVICE_TYPE, airTunesServerName, airTunesPort, 0, 0, airTunesMDNSProps());
